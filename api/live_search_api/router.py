...
async def get_live_search(
    ...
    if lr_id == -1:
        lr_id = (await session.execute(select(ListLrSearchSystem.id).where(and_(ListLrSearchSystem.list_id == list_id,
    ...
    query_count = (await session.execute(
        select(UserQueryCount.query_count).where(UserQueryCount.user_id == user.id))).scalars().first()

    region_list = (await session.execute(select(ListLrSearchSystem).where(and_(ListLrSearchSystem.list_id == list_id,
                                                                               ListLrSearchSystem.search_system == search_system)))).scalars().all()

    regions = (await session.execute(select(YandexLr))).scalars().all()
    region_dict = {region.Geoid: region.Geo for region in regions}

    current_region = (
        await session.execute(select(ListLrSearchSystem.lr).where(ListLrSearchSystem.id == lr_id))).scalars().first()

    print(current_region)

    return templates.TemplateResponse("positions-table.html",
                                      {"request": request,
                                       "user": user,
                                       "config_names": config_names,
                                       "group_names": group_names,
...
async def get_live_search_list_table(
        request: Request,
        list_id: int,
        start_date: int | float = Query(None),
        end_date: int | float = Query(None),
